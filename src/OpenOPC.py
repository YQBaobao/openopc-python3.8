...
# Copyright (c) 2022 Yue Bao Bao (yqbaowo@gmail.com)
#
###########################################################################
...
if os.name == 'nt':
    try:
        ...
        # Win32 variant types
        vt = dict(
            [(pythoncom.__dict__[vtype], vtype) for vtype in list(pythoncom.__dict__.keys()) if vtype[:2] == "VT"])
...
OPC_SERVER = 'Hci.TPNServer;HwHsc.OPCServer;opc.deltav.1;AIM.OPC.1;Yokogawa.ExaopcDAEXQ.1;OSI.DA.1;' \
             'OPC.PHDServerDA.1;Aspen.Infoplus21_DA.1;National Instruments.OPCLabVIEW;RSLinx OPC Server;' \
             'KEPware.KEPServerEx.V4;KEPware.KEPServerEx.V6;Matrikon.OPC.Simulation;Prosys.OPC.Simulation'
...
def wild2regex(strings):
    """Convert a Unix wildcard glob into a regular expression"""
    return strings.replace('.', '\.').replace('*', '.*').replace('?', '.').replace('!', '^')
...
def welcome(name, host='localhost', port=7766):
    """返回欢迎信息"""

    import Pyro4.core
    server_obj = Pyro4.Proxy("PYRO:opc@{0}:{1}".format(host, port))
    return server_obj.welcome_message(name)
...
def open_client(host='localhost', port=7766):
    ...
    import Pyro4.core
    server_obj = Pyro4.Proxy("PYRO:opc@{0}:{1}".format(host, port))
    object_id = server_obj.create_client()
    OpenOPC_obj = Pyro4.Proxy(f"PYRO:{object_id}@{host}:{port}")
    return OpenOPC_obj


def close_session(obj, host='localhost', port=7766):
    """Release an OpenOPC instance in the OpenOPC Gateway"""
    import Pyro4.core
    server_obj = Pyro4.Proxy("PYRO:opc@{0}:{1}".format(host, port))
    return server_obj.release_client(obj)
...
class Client(object):
    ...
    def iread(self, tags=None, group=None, size=None, pause=0, source='hybrid', update=-1, timeout=5000, sync=False,
        ...
        def remove_items(tags):
            ...
            except pythoncom.com_error as er:
                Error_msg = 'RemoveItems: %s' % self._get_error_str(er)
                raise OPCError(Error_msg)
            ...
            for gid in range(num_groups):
                ...
                # Async Read
                    ...
                    if len(valid_tags) > 0:
                        ...
                        handles, values, qualities, timestamps = [], [], [], []
                        while tx_id != self._tx_id:
                            now = time.time() * 1000
                            if now - start > timeout:
                                raise TimeOutErrors('Callback: Timeout waiting for data')
        ...
        try:
            ...
            for gid in range(num_groups):
                ...
                for tag in tags:
                    ...
                    if include_error:  error_msgs[tag] = error_msgs[f'{tag}'].strip('\r\n')
    ...
    def iproperties(self, tags, tid=None):
        ...
        try:
            ...
            if tid is not None:
                ...
                if isinstance(tid, list) or isinstance(tid, tuple):
                    property_id = list(tid)
            ...
            for tag in tags:
                property_id, descriptions = [], []
                if tid is None:
                    count, property_id, descriptions, datatype = self._opc.QueryAvailableProperties(tag)
    ...
    def properties(self, tags, tid=None):
        ...
        if type(tags) not in (list, tuple) and type(tid) not in (type(None), list, tuple):
            single = True
        else:
            single = False

        props = self.iproperties(tags, tid)
        ...
        try:
            ...
            for path in paths:
                ...
                if flat:
                    ...
                    if include_type:  matches = [x for x in matches]
                ...
                while len(queues) > 0:
                    ...
                    if not lowest_level and recursive:
                        queues += [path_str + x + path_postfix for x in matches]
                    else:
    ...
    def ping(self):
        ...
        try:
            ...
            opc_serv_time = int(datetime.timestamp(self._opc.CurrentTime) * 1000000.0)
    ...
    def _get_error_str(self, err):
        ...
        if exc is None:
            ...
            try:
                opc_err_str = str(self._opc.GetErrorString(code)).strip('\r\n')
            except Exception:
                opc_err_str = None

            try:
                com_err_str = str(pythoncom.GetScodeString(code)).strip('\r\n')
            except Exception:
                com_err_str = None
    ...
    def __getitem__(self, key):
        ...
        value, quality, times = self.read(key)
        return value
